...
// Wrap everything in an async function to avoid top-level await
async function startServer() {
  ...
  async function gracefulShutdown(signal: string) {
    ...
    try {
      ...
      try {
        ...
        console.log(
          '⚠️ Apollo Server stop timeout, continuing with shutdown...',
        )
      }
  ...
  // Handle IPC shutdown message (preferred for Electron)
  process.on('message', (message) => {
    if (message === 'shutdown') {
      console.log('📨 Received IPC shutdown message')
      gracefulShutdown('IPC').catch(console.error)
    }
  })

  // Handle shutdown signals (fallback)
  process.on('SIGINT', () => {
