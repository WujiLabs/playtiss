...
const tempDb = join(tempDir, 'playtiss.db')
...
try {
  ...
  exec(`sqlite3 "${tempDb}" "PRAGMA wal_checkpoint(TRUNCATE);"`)

  // Copy the DB
  const outputDb = join(PROJECT_ROOT, 'graphql-server', 'playtiss-test-add3.db')
  cpSync(tempDb, outputDb)
  ...
  console.log(`  pipeline-runner/src/index.ts  MONITORED_PIPELINES = ['${addThreeId}']`)
