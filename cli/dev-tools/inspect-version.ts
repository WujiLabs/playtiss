...
async function inspectVersion(versionId: string) {
  ...
  try {
    ...
    const version = db.prepare('SELECT * FROM Versions WHERE version_id = ?').get(versionId) as VersionRow | undefined
    ...
    const task = db.prepare('SELECT * FROM Tasks WHERE task_id = ?').get(version.task_id) as TaskRow | undefined
    ...
    // Get child versions
    const childVersions = db.prepare('SELECT * FROM Versions WHERE parent_version_id = ? ORDER BY timestamp_created DESC').all(versionId) as VersionRow[]
    ...
    if (version.version_type_tag === 'workflow_definition') {
      const executions = db.prepare('SELECT * FROM Versions WHERE executed_def_version_id = ? ORDER BY timestamp_created DESC').all(versionId) as VersionRow[]
